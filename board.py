...
#         self.weights = weights
    ...
    @classmethod
    def init_board(self, size,weights=0):
        ...
        for x in range(1, size - 1):
            ...
            board[x][size - 1] = 2
#         print(board)
        return Game_Board(board, size, 1)#,weights)
...
#             print()
        ...
        return list(map(lambda b: Game_Board(b, self.size, 3- self.to_move), boards))

    def get_moves_for_square(self, x, y):
        
        def get_move( x, y, x_dir, y_dir, move_size):
            x_pos = x
            y_pos = y
        ...
        def get_diagonal_1_move_size(x, y):
            move_size = 1
            x_pos = x
            y_pos = y
        ...
        def get_diagonal_2_move_size(x, y):
            move_size = 1
            x_pos = x
            y_pos = y
            for i in range(self.size):
...
#             print(h_move_size)
            ...
            d1_move_size = get_diagonal_1_move_size(x, y)
            d2_move_size = get_diagonal_2_move_size(x, y)

            boards += get_move(x, y, 0, 1, h_move_size)
            boards += get_move(x, y, 0, -1, h_move_size)
            boards += get_move(x, y, 1, 0, v_move_size)
            boards += get_move(x, y, -1, 0, v_move_size)
            boards += get_move(x, y, 1, 1, d1_move_size)
            boards += get_move(x, y, -1, -1, d1_move_size)
            boards += get_move(x, y, 1, -1, d2_move_size)
            boards += get_move(x, y, -1, 1, d2_move_size)
